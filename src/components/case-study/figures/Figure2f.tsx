    ...
    <ScalingContainer designWidth={700}>
      <motion.div
        ...
        <thead>
          <tr>
            <motion.th 
              className="border border-foreground/40 p-2.5 text-left font-display text-sm bg-foreground/[0.07] w-[100px]"
            ...
            {stages.map((stage, idx) => (
              <motion.th 
                key={idx} 
                className={`border border-foreground/40 p-2.5 text-center font-display text-sm ${
        ...
        <tbody>
          {journeyRows.map((row, rowIdx) => (
            ...
            >
              <motion.td 
                className={`border border-foreground/40 p-2.5 font-display text-sm align-top bg-foreground/[0.04] ${
              ...
              {row.cells.map((cell, cellIdx) => (
                <motion.td 
                  key={cellIdx} 
                  className={`border border-foreground/40 p-2.5 font-display text-sm align-top leading-relaxed ${
                    row.isHighlight ? "text-red-600" : ""
                  }`}
                  whileHover={{ 
