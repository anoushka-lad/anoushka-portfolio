...
const Figure2e = () => {
  ...
  const TableRow = ({ category, content, index }: { category: string; content: string[]; index: number }) => (
    ...
    >
      <td className="border border-foreground/40 p-2 font-serif font-bold text-[10px] md:text-xs align-top w-[35%] leading-tight">
        {category}
      </td>
      <td className="border border-foreground/40 p-2 text-[10px] md:text-xs align-top">
        <ul className="list-disc list-outside ml-4 space-y-0.5">
          {content.map((item, i) => (
            <li key={i} className="font-serif leading-tight">{item}</li>
          ))}
        </ul>
      </td>
    </motion.tr>
  ...
  const SectionHeader = ({ title }: { title: string }) => (
    ...
    >
      <td colSpan={2} className="border border-foreground/40 p-2 font-serif font-bold text-xs md:text-sm bg-foreground/[0.07]">
        {title}
      </td>
    </motion.tr>
  );
          ...
          <table className="w-full border-collapse text-foreground h-full">
            <tbody>
              ...
              <motion.tr whileHover={{ backgroundColor: "rgba(0,0,0,0.03)" }}>
                <td colSpan={2} className="border border-foreground/40 p-2 font-serif font-bold text-[10px] md:text-xs bg-foreground/[0.04]">
                  Key Insights
                </td>
              </motion.tr>
              <motion.tr whileHover={{ backgroundColor: "rgba(0,0,0,0.03)" }}>
                <td className="border border-foreground/40 p-2 font-serif font-bold text-[10px] md:text-xs align-top w-[30%]">
                  Key Findings
                </td>
                <td className="border border-foreground/40 p-2 text-[10px] md:text-xs align-top">
                  <ul className="list-disc list-outside ml-4 space-y-0.5">
              ...
              <motion.tr whileHover={{ backgroundColor: "rgba(0,0,0,0.03)" }}>
                <td className="border border-foreground/40 p-2 font-serif font-bold text-[10px] md:text-xs align-top">
                  Goals
                </td>
                <td className="border border-foreground/40 p-2 text-[10px] md:text-xs align-top">
                  <ul className="list-disc list-outside ml-4 space-y-0.5">
              ...
              <motion.tr whileHover={{ backgroundColor: "rgba(0,0,0,0.03)" }}>
                <td className="border border-foreground/40 p-2 font-serif font-bold text-[10px] md:text-xs align-top bg-foreground/[0.04]">
                  Summary
                </td>
                <td className="border border-foreground/40 p-2 text-[9px] md:text-[10px] align-top">
                  <p className="font-serif leading-relaxed">{rightColumnData.summary}</p>
                </td>
              </motion.tr>
            </tbody>
          </table>
